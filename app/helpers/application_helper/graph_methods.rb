...
module ApplicationHelper::GraphMethods
  ...
  def user_twitter_card(user)
    meta_tags({
                          ...
                          "twitter:description":  "Ask #{user.profile.safe_name} anything on Revospring",
                          "twitter:image":        preview_user_image_url_for(user),
                          "twitter:site":         "@revospring",
              })
  end
  ...
  def answer_twitter_card(answer)
    meta_tags({
                          "twitter:title":        "#{answer.user.profile.safe_name} answered: #{answer.question.content}",
                          "twitter:card":         "summary",
                          "twitter:description":  answer.content,
